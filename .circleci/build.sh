...
echo "==> Symlink profile code"
rm -rf build/web/profiles/"${MODULE}"/* > /dev/null
mkdir -p "build/web/profiles/${MODULE}"
ln -s "$(pwd)"/* build/web/profiles/"${MODULE}" && rm build/web/profiles/"${MODULE}"/build

echo "==> Install Drupal into SQLite database ${DB_FILE}"
php -d memory_limit=-1 "$(command -v build/vendor/bin/drush)" -r build/web si "${DRUPAL_PROFILE:-bt_store_cl}" -y --db-url "sqlite://${DB_FILE}" --account-name=admin install_configure_form.enable_update_status_module=NULL install_configure_form.enable_update_status_emails=NULL install_configure_form.site_default_country=CL install_configure_form.date_default_timezone='America/Santiago' install_configure_form.store_name='Tienda Demo' install_configure_form.store_mail='tiendademo@demo.cl' install_configure_form.address='Lejos Lejos Super Lejos' install_configure_form.region='Región Metropolitana' install_configure_form.comuna='El Bosque'
